...
class WallTilePicker:
    """Neighbor bitmask lookup for maze wall sprite selection."""

    BY_NEIGHBORS: ClassVar[dict[tuple[bool, bool, bool, bool], TileKind]] = {
    ...
    def pick_for_neighbors(
        ...
        return WallTilePicker.BY_NEIGHBORS.get(
            (up, down, left, right),
            TileKind.WALL,
        )
    ...
    def pick(layout: LevelLayout, cell: CellPos) -> TileKind:
        ...
        up = WallTilePicker._has_wall_neighbor(
            layout, CellPos(cell.row - 1, cell.col)
        )
        down = WallTilePicker._has_wall_neighbor(
            layout, CellPos(cell.row + 1, cell.col)
        )
        left = WallTilePicker._has_wall_neighbor(
            layout, CellPos(cell.row, cell.col - 1)
        )
        right = WallTilePicker._has_wall_neighbor(
            layout, CellPos(cell.row, cell.col + 1)
        )
        return WallTilePicker.pick_for_neighbors(up, down, left, right)
